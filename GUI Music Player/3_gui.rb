...
class MusicPlayerMain < Gosu::Window
  ...
  def area_clicked(mouse_x, mouse_y)
	...
	if (mouse_y > 50 && mouse_y < 250)
		if (mouse_x > 0 && mouse_x < 300)
			@current_album = 1
		elsif (mouse_x > 320 && mouse_x < 620)
			@current_album = 2
		else
			@current_album = 0
		end
 ...
 def draw
	...
	if @current_album != 0 
	  ...
	  if (!@song)
		@font.draw("#{albums[@current_album-1].title}", x, y += 100, ZOrder::UI, 1.0, 1.0, Gosu::Color::BLACK)
	  else
		i = 0
	...
	def select_track()
		if (mouse_x > 700 && mouse_x < 1050) 
			...
			elsif (mouse_y > 300 && mouse_y < 350)
				return @current_track = 2
			elsif (mouse_y > 350 && mouse_y < 400)
				return @current_track = 3
			elsif (mouse_y > 400 && mouse_y < 450)
				return @current_track = 4
			elsif (mouse_y > 450 && mouse_y < 500)
