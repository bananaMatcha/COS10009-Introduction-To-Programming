...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    y_cell_count = MAP_HEIGHT / CELL_DIM #10

    @columns = Array.new(x_cell_count)
    column_index = 0
# x = column, y = row 

# x_cell_count = total cell per column = 10
# y_cell_count = total cell per row = 10

#column_index = CURRENT x
#row_index = CURRENT y

    # first create cells for each position
    while (column_index < x_cell_count) 
      row = Array.new(y_cell_count) #array for one row 
      @columns[column_index] = row # this row is belong to 1 x coordinate 
      row_index = 0
      while (row_index < y_cell_count)
        cell = Cell.new() 
        @columns[column_index][row_index] = cell #each cell is an item in a multi-dimension array
        row_index += 1
      end
      column_index += 1
    end
    ...
    while (column_index < x_cell_count) 
      ...
      #cell from the left
        if (column_index == 0) #at the right edge of the table
          cell.west = nil
        else  
          cell.west = @columns[column_index - 1][row_index] #the cell on the right of the current cell
        end
      #cell from the right
        if (column_index == (x_cell_count -1)) #column_index = [0;9] and  x_cell_count =10
          cell.east = nil
        else
         ...
         if (row_index == (y_cell_count -1)) #at the bottom edge (south)
          cell.south = nil
        else  
          cell.south = @columns[column_index][row_index + 1] #this neighbour cell is at south of the current cell
        end
     ...
     column_index +=1
      ...
      print_neighbour_links(@columns)
    end
end
    def print_neighbour_links(array)
      ...
      while (column_index < x_cell_count)
          ...
          while (row_index < y_cell_count)
                ...
                print (" north:" + check_if_neighbour_is_nil(array[column_index][row_index].north).to_s)
                print (" south:" + check_if_neighbour_is_nil(array[column_index][row_index].south).to_s)
                print (" east:" + check_if_neighbour_is_nil(array[column_index][row_index].east).to_s)
                print (" west:" + check_if_neighbour_is_nil(array[column_index][row_index].west).to_s)
                print ("\n")
    ...
    def check_if_neighbour_is_nil (cell) # cell = @columns[column_index][row_index] 
       if (cell == nil)
        return 0
       else 
        return 1 
  ...
  def mouse_over_cell(mouse_x, mouse_y)
    if mouse_x <= CELL_DIM # the mouse is at the leftmost cell
      cell_x = 0
    else
      cell_x = (mouse_x / CELL_DIM).to_i #works out which cell the mouse is at
    end
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1)) # = (x_cell_count -1) ,as cell_x = [0;9]
      ...
      if @columns[cell_x][cell_y-1].vacant && !@columns[cell_x][cell_y-1].visited 
        north_path = [cell_x,cell_y - 1]
      end
      if  @columns[cell_x + 1][cell_y].vacant && !@columns[cell_x+1][cell_y].visited 
