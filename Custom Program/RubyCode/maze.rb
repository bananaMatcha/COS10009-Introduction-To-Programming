...
class GameWindow < Gosu::Window
  ...
  def print_cells(array)
    ...
    while (column_index < x_cell_count)
      ...
      while (row_index < y_cell_count)
        ...
        print (" north:" + check_if_nil(array[column_index][row_index].north).to_s)
        print (" south:" + check_if_nil(array[column_index][row_index].south).to_s)
        print (" east:" + check_if_nil(array[column_index][row_index].east).to_s)
        print (" west:" + check_if_nil(array[column_index][row_index].west).to_s)
        print ("\n")
        row_index += 1
      end
      puts "---------- End of column ----------"
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      if ((@columns[cell_x][cell_y].north != nil) and 
          @columns[cell_x][cell_y].north.vacant and
          !@columns[cell_x][cell_y].visited)
            @columns[cell_x][cell_y].north.visited = true
            north_path = search(cell_x, cell_y - 1) #path maybe nill
      end

      if ((@columns[cell_x][cell_y].south != nil) and 
          @columns[cell_x][cell_y].south.vacant and
          !@columns[cell_x][cell_y].visited)
            @columns[cell_x][cell_y].south.visited = true
            south_path = search(cell_x, cell_y - 1) #path maybe nill
      end

      if ((@columns[cell_x][cell_y].east != nil) and 
          @columns[cell_x][cell_y].east.vacant and
          !@columns[cell_x][cell_y].visited)
            @columns[cell_x][cell_y].east.visited = true
            east_path = search(cell_x, cell_y - 1) #path maybe nill
      end

      if ((@columns[cell_x][cell_y].west != nil) and 
          @columns[cell_x][cell_y].west.vacant and
          !@columns[cell_x][cell_y].visited)
            @columns[cell_x][cell_y].west.visited = true
            west_path = search(cell_x, cell_y - 1) #path maybe nill
      end
